...
if ideal_observer == 1;
    ...
    lower_bounds = [0 0 .7 .7 0 0];
    upper_bounds = [1 1  .7  .7 1 1];
...
% means = plot_sequence_behaviour(data);
...
function [sus_means_p_ss key_ss] = reformat_sequence_data(data)
...
sus_means_p_ss = nan(num_pids * 4, 11);  % 416x11 matrix
    ...
    for suspect = 0:1
        for context = 0:1
            ...
            for seqPos = 0:10
                ...
                mean_value = mean(subset.mean_ModelProbability(subset.SequencePosition == seqPos));  % Replace with the column you want to calculate mean for
                
                % Store the mean value in the matrix
                sus_means_p_ss(rowIndex, seqPos + 1) = mean_value;
            end
...
% %Takes means and confidence intervals computed for sequence positions, contexts and suspects for the
% %prior model probabilities. These are stored in a data table but we want
% %the data to be in the same formagt I was using to plot the human and
% %participant data in another script. So this reformats the data from a
% %table to matrices so they can be plugged into the plots there.
% 
% 
...
% dataTable = grpstats(temp,{'Suspect', 'Context','SequencePosition'},{'mean' 'meanci'});
% 
% for suspect = 0:1
%     for context = 0:1
%         % Calculate row index based on suspect and context
%         rowIndex = suspect * 2 + context + 1;
%         
%         for seqPos = 0:10
%             % Extract the relevant subset of the table
%             subset = dataTable(dataTable.Suspect == suspect & dataTable.Context == context & dataTable.SequencePosition == seqPos, :);
%             
%             % Populate the matrices
% %             meanAdjustMatrix(rowIndex, seqPos + 1) = subset.mean_mean_ModelAdjust;
% %             meanciAdjustMatrix(rowIndex, seqPos + 1) = subset.meanci_mean_ModelAdjust;
%             sus_means_p(rowIndex, seqPos + 1) = subset.mean_mean_ModelProbability;
%             sus_cis_p(rowIndex, seqPos + 1,1) = subset.meanci_mean_ModelProbability(1);
%             sus_cis_p(rowIndex, seqPos + 1,2) = subset.meanci_mean_ModelProbability(2);
%         end
%     end
% end
...
% function means = plot_sequence_behaviour(data);
% 
