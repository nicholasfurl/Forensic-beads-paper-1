...
for participant = 1:num_participants;
    ...
    for suspect = 1:num_suspects;
        ...
        [params_est(participant, suspect) ll(participant, suspect) flag search] = ...
            fminsearchbnd( @(params) get_model_ll(params, this_ps_suspect_data), params,[0],[1]);
...
this_ps_suspect_data = get_model_behaviour(prior,this_ps_suspect_data);
...
function this_ps_suspect_data = get_model_behaviour(prior, this_ps_suspect_data)
...
for seq = 1:numel(seq_start_indices);
    ...
    for claim = 1:11;
        ...
        if claim == 1;
            ...
            ng = sum( this_ps_suspect_data(seq_start_indices(seq)+1:index,2) );
