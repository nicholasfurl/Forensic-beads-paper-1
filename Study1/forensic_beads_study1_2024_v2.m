...
%forensic_beads_study1_2024_v2: I am modifying
%forensic_beads_study2_2024_v3 (as it takes two suspect parameters per
%participant already) to take the study 1 data as one huge participant. 
%forensic_beads_study1_2024 already fits models for study 1 participant by
%participant, though that method doesn't seem to fare well during parameter
%recovery so am trying this way.
...
model_Pid = ones(size(data.Pid,1),1);
model_participant_list = unique(model_Pid,'stable');
model_num_participants = numel(model_participant_list);
...
%recency
var_names = {'Pid','Loss','Prior atheist','Prior Christian','Window','Bias','Noise'};
...
%delta
var_names = {'Pid','Loss','Prior atheist','Prior Christian','Alpha','Beta'};
...
%split
var_names = {'Pid','Loss','Prior atheist','Prior Christian','Split','Bias','Noise'};
...
for participant = 1:model_num_participants;
...
%         :);
    ...
    this_ps_data = ...
        data( ...
        model_Pid == model_participant_list(participant),...
        :);
    ...
    %fitted parameters constrained to be between these values
    model_struct(model_num).lower_bounds = [0 0 .1 1];
    model_struct(model_num).upper_bounds = [1 1 1 150];
    ...
    params(1) = .5; %prior, initialised to optimal value (ground truth of paradigm)
    params(2) = .5; %prior, initialised to optimal value (ground truth of paradigm)
    params(3) = .6;  %split term, initialised to optimal value
    params(4) = 0;  %bias term, intialised to optimal value
    params(5) = 1;  %noise term, initialised to optimal value

    %fitted parameters constrained to be between these values
    model_struct(model_num).lower_bounds = [0 0 .5 0 0];
    model_struct(model_num).upper_bounds = [1 1  1 1 1];
...
winning_model_diffs = winning_model_struct.model_fitting_results{:,'Prior atheist'} -  winning_model_struct.model_fitting_results{:,'Prior Christian'};

%trad paired t-test
[th tpvals tci tstats] =   ...   
    ttest(winning_model_diffs);

%paired Bayesian test
[bf10,bfpvals,bfci,bfstats] = ...
        bf.ttest( winning_model_diffs);
...
for plot = 1:2; %innocent and guilty sequences
    ...
    for i=its;    %atheist and Christian lines
        ...
        for j=1:size(plot_details.cis,2);   %sequences positions
            ...
            end;    %If plotting the top line (atheist)
...
function sequence_probabilities = get_behaviour_for_this_ps_sequences(new_params,model_struct);

%The first two params MUST BE suspect 0 prior and suspect 1 prior. Then
%other parameters
...
for seq = 1:numel(seq_start_indices);
    ...
    this_params = [new_params(this_suspect_code+1) new_params(3:end)];  %pick out the prior parameter for the correct suspect, know out the other suspect, and keep the rest of the (not specifric to any suspect) parametesr
...
%fit params to data
    ...
    model_struct.model_fitting_results = ...
        [model_struct.model_fitting_results; ...
        num2cell([model_struct.participant new_loss new_params])
        ];
...
prior = params(1);
split = params(2);
response_bias = params(4);
response_noise = params(5);
...
draws = this_ps_suspect_data.Claim(2:end);
num_claims = size(draws,1);  %first row is prior rating and has no claim

q_hat = prior;
q_hat_list = nan(num_claims,1);
model_probabilities = nan(num_claims,1);

% %For each start index, loop through sequence and get model predictions
% for seq = 1:numel(seq_start_indices);

%Loop through this sequence
for claim = 1:num_claims;
...
%     else;
        ...
        q_hat_list(claim,1) = q_hat;
        model_probabilities(claim,1) = exp(beta*q_hat)/(exp(beta*q_hat) + exp(beta*(1-q_hat)));

end;    %loop through this sequence (claim)

%add prior back into model probability vector so it aligns with participant ratings
model_probabilities = [prior; model_probabilities];
% end;    %loop through sequences
...
for model = 1:num_models;
    ...
    N = 599*11;  %number of probability judgments per fitted participants 
...
[dummy, winning_model_it] = min(mean(fit_vals,1));
